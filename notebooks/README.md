...
| `drug_catalog.ipynb` | Cross-database compound harmonization (**CTRP / PRISM / DrugBank**). Writes `data/drug/all_sources_drug_catalog.csv`, which [`2_drug_selection`](2_drug_selection.ipynb) reads for approval status, target and mechanism. **Rewritten 13.08.2026:** every path now resolves through `PipelinePaths`, so it runs in a worktree and on another machine; §6 reads DrEval's response table instead of CTRPv2's retired `v20.data.per_cpd_post_qc.txt`; and the **GDSC arm was removed** (Selin) — an artifact of a one-off learnability analysis, and the drug selection must not be done on GDSC. It also read a catalog path that did not exist (`../data/` rather than `../data/drug/`), so it raised at §5 | `scripts/annotation/drug_annotation.py` |
