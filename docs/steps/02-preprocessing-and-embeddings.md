...
data/
  ...
  scRNAseq_SCP542/other/UMIcount_data.txt       # un-normalised; depth + mito only (13.08.2026)
  ...
  processed/scRNAseq_SCP542/umi_qc_covariates.csv   # cached, shared by both variants
...
- raw UMI counts → `data/scRNAseq_SCP542/other/UMIcount_data.txt` — **read by one step only**
  (`qc_covariates.py`, 30,314 genes × 56,982 cells). The CPM matrix has library size divided out, so
  this is the sole source of `total_counts` and `pct_counts_mt`.
...
`processed/scRNAseq_SCP542/umi_qc_covariates.csv` sits **beside** the variant directories, not inside
one: the covariates are computed over the full gene set, so both variants produce identical values and
a per-variant cache would scan 3.5 GB twice for one answer. It is regenerated by `convert` whenever
absent, so a rerun needs nothing done by hand.
