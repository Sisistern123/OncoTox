...
%     BUT NOTE: the drug-panel funnel typed inline in 03_methods.tex WAS
%     computed on 181 -- 2_drug_selection.ipynb reads the response CSV directly,
%     not a pipeline artifact, so it already uses the corrected join
%     (doxorubicin: n_auc_cc 177 at coverage 0.9779 => 181). So the report
%     currently states 180 in Data and 181-derived coverage in Drug panel,
%     without saying so. Reconcile at R6; do not "fix" either half alone.
...
%   * \NIndep (150) is used for TWO different quantities and matches neither
%     exactly: the effective sample size after the held-out splits, which is
%     153 lines (70+15 percent of 180), and the line count of the ridge control,
%     which was 150 because that run's drugs were not labelled on every line.
%     Its one misuse -- standing in for the 153 CV lines in 03_methods.tex
%     Evaluation -- was removed 12.08.2026; the macro itself needs a decision on
%     which quantity it names before R6 rewrites the sections that cite it.
%
